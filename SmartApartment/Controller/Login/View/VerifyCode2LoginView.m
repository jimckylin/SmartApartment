...
@interface VerifyCode2LoginView ()<UITextFieldDelegate>
...
@property (nonatomic, strong) RTLabel         *tipLabel;  // 提示文字
...
@implementation VerifyCode2LoginView
...
- (void)initUI {
    ...
    [_settingPwdBtn setTitle:@"登录" forState:UIControlStateNormal];
    ...
    _tipLabel = [[RTLabel alloc] initWithFrame:CGRectZero];
    [_tipLabel setTextColor:[UIColor lightGrayColor]];
    [_tipLabel setFont:[UIFont systemFontOfSize:11.0f]];
    [_tipLabel setText:[NSString stringWithFormat:@"动态密码将发送至<font color=red>%@</font>", self.phone]];
    [_tipLabel setUserInteractionEnabled:YES];
    [self.loginBgView addSubview:_tipLabel];
}
...
- (void)verifyCodeBtnClick:(id)sender {
    ...
    [self startCountDown];
    if ([self.delegate respondsToSelector:@selector(verifyCode2ViewVerifyCodeBtnClick)]) {
        [self.delegate verifyCode2ViewVerifyCodeBtnClick];
    }
}
...
- (void)btnClick:(id)sender {
    ...
    if ([self.delegate respondsToSelector:@selector(verifyCode2LoginView:)]) {
        [self.delegate verifyCode2LoginView:_verifyCodeTF.text];
    }
}
...
- (void)updateConstraints {
    ...
    [self.tipLabel autoPinEdge:ALEdgeTop toEdge:ALEdgeBottom ofView:self.settingPwdBtn withOffset:15];
    [self.tipLabel autoAlignAxisToSuperviewAxis:ALAxisVertical];
    [self.tipLabel autoSetDimension:ALDimensionHeight toSize:20];
