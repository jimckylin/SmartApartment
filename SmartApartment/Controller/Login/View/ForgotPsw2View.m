...
@interface ForgotPsw2View ()<UITextFieldDelegate>
...
- (void)initUI {
    ...
    [_settingPwdBtn setTitle:@"重置密码" forState:UIControlStateNormal];
...
- (void)btnClick:(id)sender {
    ...
    if ([Utils isBlankString:_verifyCodeTF.text] || [_verifyCodeTF.text length] < 4 || [_verifyCodeTF.text length] > 20) {
        [MBProgressHUD cwgj_showHUDWithText:@"请输入正确格式密码"];
    }
    
    if ([self.delegate respondsToSelector:@selector(forgotPsw2ViewDidClickResetBtnAction:)]) {
        [self.delegate forgotPsw2ViewDidClickResetBtnAction:_verifyCodeTF.text];
    }
}
