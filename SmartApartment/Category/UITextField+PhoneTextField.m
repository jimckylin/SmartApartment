...
static const int PHONE_COUNT = 13;
...
{
    NSString *res = nil;
    if (self.text.length != PHONE_COUNT) {
    ...
    NSString * fStr = [self.text substringToIndex:3];
    [string appendString:fStr];
    NSString * mStr = [self.text substringWithRange:NSMakeRange(4, 4)];
    [string appendString:mStr];
    NSString * lStr = [self.text substringFromIndex:9];
    [string appendString:lStr];
...
{
    NSString *pattern = @"^((13[0-9])|(14[5,7])|(15[^4,\\D])|(17[0,1,3,5-8])|(18[0-9]))\\d{8}$";
